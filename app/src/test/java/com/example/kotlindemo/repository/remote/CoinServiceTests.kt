...
class CoinServiceTests {
    ...
    private fun verifyCoinDetails(coinDetails: CoinDetail) {
        ...
        assertEquals("Working product", coinDetails.development_status)
        assertEquals("2010-07-17T00:00:00Z", coinDetails.first_data_at)
        assertEquals(true, coinDetails.hardware_wallet)
        assertEquals("SHA256", coinDetails.hash_algorithm)
        assertEquals("btc-bitcoin", coinDetails.id)
        assertEquals(true, coinDetails.is_active)
        assertEquals(false, coinDetails.is_new)
        assertEquals("2021-10-19T20:25:00Z", coinDetails.last_data_at)
        coinDetails.links?.let { verifyLinks(it) }?: fail()
        coinDetails.links_extended?.let { verifyLinksExtended(it) }?: fail()
        assertEquals("", coinDetails.message)
        assertEquals("Bitcoin", coinDetails.name)
        assertEquals(true, coinDetails.open_source)
        assertEquals("Decentralized", coinDetails.org_structure)
        assertEquals("Proof of Work", coinDetails.proof_type)
        assertEquals(1, coinDetails.rank)
        assertEquals("2009-01-03T00:00:00Z", coinDetails.started_at)
